...
import os, sys, struct, random, math, time, socket, traceback, logging
from ...common import getlocaladdr
from ... import multitask

logger = logging.getLogger('rfc3550')
...
def binary(data, size=4):
    all = ''.join([''.join([('1' if (x & (1 << (7 - y))) else '0') for y in range(0, 8)]) for x in data])
...
class RTP(object):
    ...
    >>> print(''.join(['%02x'%x for x in str(p1)]))
    ...
    def __init__(self, value=None, pt=0, seq=0, ts=0, ssrc=0, csrcs=None, marker=False, extn=None, payload=''):
        ...
        if not value:  # construct using components.
            ...
            self.csrcs, value = ([] if (px & 0x0f == 0) else list(
                struct.unpack('!' + 'I' * (px & 0x0f), value[12:12 + (px & 0x0f) * 4]))), value[12 + (px & 0x0f) * 4:]
            if px & 0x10:
                xtype, xlen = struct.unpack('!HH', value[:4])
                self.extn, value = (xtype, value[4:4 + xlen * 4]), value[4 + xlen * 4:]
            else:
                self.extn = None
            self.payload = value if px & 0x20 == 0 else value[:len(value) - value[-1]]

    def __repr__(self):
        global _padding
        return struct.pack('!BBHII', 0x80 | (_padding and (len(self.payload) % 4 != 0) and 0x20 or 0x00) | (
                    self.extn and 0x10 or 0x00) | (len(self.csrcs) > 15 and 15 or len(self.csrcs)), \
                           (self.pt & 0x7f) | (self.marker and 1 or 0) << 7, (self.seq & 0xffff), self.ts, self.ssrc) \
               + ''.join(struct.pack('!I', x) for x in self.csrcs[:16]) \
               + ('' if not self.extn else (
                    struct.pack('!HH', self.extn[0] & 0xffff, len(self.extn[1]) / 4) + self.extn[1])) \
               + self.payload \
               + ('' if (not _padding or len(self.payload) % 4 == 0) else (
                    '\x00' * (4 - len(self.payload) % 4 - 1) + struct.pack('!B', 4 - len(self.payload) % 4)))
...
class RTCP(list):
    ...
    def __init__(self, value=None):  # parse the compound RTCP packet.
        ...
        while value and len(value) > 0:
            ...
            if px & 0x20: data = data[:len(data) - data[-1]]  # remove padding
            if p.pt == RTCP.SR or p.pt == RTCP.RR:
                ...
                if data and len(data) > 0:
                    ...
                    p.reason = data[
                               1:1 + rlen]  # no need to ignore padding, it already gets ignored when we use next packet
            elif p.pt == RTCP.APP:
                p.subtype = px & 0x1f
        ...
        for p in self:
            ...
            if p.pt == RTCP.SR or p.pt == RTCP.RR:
                if p.pt == RTCP.SR:
                    ...
                    value = struct.pack('!I', p.ssrc)
                count = len(p.reports)
                for r in p.reports:
                    value += struct.pack('!IIIIII', r.ssrc, (r.flost << 24) | (r.clost & 0x0ffffff), r.hseq, r.jitter,
                                         r.lsr, r.dlsr)
                ...
                if p.reason and len(p.reason) > 0: value += struct.pack('!B', len(p.reason) > 255 and 255 or len(
                    p.reason)) + p.reason[:256]
            elif p.pt == RTCP.APP:
                count = p.subtype
            ...
            result += struct.pack('!BBH', 0x80 | (_padding and len(value) % 4 != 0 and 0x20 or 0x00) | (count & 0x1f),
                                  p.pt, length) \
                      + value + ('' if (not _padding or len(value) % 4 == 0) else (
                        '\x00' * (4 - len(value) % 4 - 1) + struct.pack('!B', 4 - len(value) % 4)))
...
class Source(object):
    ...
    def updateseq(self, seq):
        ...
        if self.probation > 0:
            ...
            if seq == self.badseq:
                self.initseq(seq)  # probably the other side reset the seq
            else:
    ...
    def updatelostandexpected(self):
        ...
        if expectedinterval == 0 or lostinterval <= 0:
            self.fraction = 0
        else:
            self.fraction = (lostinterval << 8) / expectedinterval
        return self
...
class Session(object):
    ...
    def __init__(self, app, **kwargs):
        ...
        self.app, self.net, self.pt, self.rate, self.bandwidth, self.fraction, self.member = \
            app, None, kwargs.get('pt', 96), kwargs.get('rate', 8000), kwargs.get('bandwidth', 64000), kwargs.get(
                'fraction', 0.05), kwargs.get('member', None)
        if not self.member:
    ...
    def randint(self, low=0, high=0x100000000):
        ...
        return random.randint(low,
                              high)  # TODO: use the algorithm defined in RFC to implement this instead of using random
    ...
    def start(self):
        ...
        self.senders.clear();
        self.members.clear();  # add ourself in members.
        self.pmembers = 1
        ...
        if hasattr(self.app, 'starting') and callable(self.app.starting): self.app.starting(
            self)  # ignore if starting() is not defined
    ...
    def stop(self, reason=''):
        ...
        if self.timer:
            ...
            self.timer = None
        self.running = False
        if hasattr(self.app, 'stopping') and callable(self.app.stopping): self.app.stopping(
            self)  # ignore if stopping is not defined
        self.net = None
    ...
    def send(self, payload='', ts=0, marker=False, pt=None):
        ...
        pkt = RTP(pt=pt, marker=marker, seq=self.seq0 + self.seq, ts=(self.ts0 + ts) & 0xffffffff, ssrc=member.ssrc,
                  payload=payload)
        data = str(pkt)
        if self.net is not None:
            self.net.sendRTP(
                data)  # TODO: not a generator, multitask.add(self.net.sendRTP(data)) # invoke app or net to send the packet
        elif hasattr(self.app, 'sendRTP') and callable(self.app.sendRTP):
            self.app.sendRTP(self, data)
        else:
            logger.debug('ignoring send RTP')
    ...
    def receivedRTP(self, data, src, dest):
        ...
        if member:
            ...
            if hasattr(self.app, 'received') and callable(self.app.received):
                self.app.received(member, p)
            else:
                logger.debug('ignoring received RTP')
    ...
    def rtcpinterval(self, initial=False):
        if len(self.senders) < 0.25 * len(self.members):
            if self.wesent:
                C, n = self.avgrtcpsize / (0.25 * self.rtcpbw), len(self.senders)
            else:
                C, n = self.avgrtcpsize / (0.75 * self.rtcpbw), len(self.members) - len(self.senders)
        else:
            C, n = self.avgrtcpsize / self.rtcpbw, len(self.members)
    ...
    def sendRTCP(self, sendbye=False):
        ...
        p = RTCP.packet(pt=RTCP.SDES,
                        items=self.member.items)  # add SDES. Should add items only every few packets, except for CNAME which is added in every.
        packet.append(p)
        ...
        # TODO: calling multitask and no gevent here.
        if self.net is not None:
            ...
            logger.debug('ignoring send RTCP')
        self.rtcpsent = True
        return len(data)
...
class Network(object):
    ...
    def _initialize(self, app, **kwargs):
        ...
        if self.src[1] != 0:  # specified port
            try:
                ...
                logger.debug('created RTP/RTCP sockets %r %r', s1, s2)
                s1.bind(self.src)
                s2.bind(self.srcRTCP)
            except:
                logger.exception('failed to bind. closing %r %r', s1, s2)
                s1.close();
                s2.close();
                s1 = s2 = None
        else:
            ...
            while retry > 0:
                s1 = socket.socket(type=socket.SOCK_DGRAM)
                s2 = socket.socket(type=socket.SOCK_DGRAM)
                logger.debug('created RTP/RTCP sockets(2) %r %r', s1, s2)
                ...
                try:
                    ...
                    logger.exception('failed to bind. closing(2) %r %r', s1, s2)
                    s1.close();
                    s2.close();
                    s1 = s2 = None
                retry = retry - 1
    ...
    def close(self):
        logger.debug('cleaning up sockets %r %r', self.rtp, self.rtcp)
    ...
    def receiveRTP(self, sock):
        try:
            ...
            logger.exception('receive RTP exception')
        try:
            os.close(fd)
        except:
            pass
    ...
    def receiveRTCP(self, sock):
        try:
            ...
            logger.exception('receive RTCP exception')
        try:
            os.close(fd)
        except:
            pass
    ...
    def sendRTP(self, data, dest=None):  # unline sendRTCP this is not a generator
        if self.rtp:
            ...
            if dest and dest[1] > 0 and dest[0] != '0.0.0.0':
                logger.debug('sending RTP %d to %r', len(data), dest)
    ...
    def sendRTCP(self, data, dest=None):
        if self.rtcp:
            ...
            if dest and dest[1] > 0 and dest[0] != '0.0.0.0':
                logger.debug('sending RTCP %d to %r', len(data), dest)
...
class gevent_Network(Network):
    ...
    def close(self):
        logger.debug('cleaning up sockets %r %r', self.rtp, self.rtcp)
        # contributed by luke.weber@gmail.com, p2p-sip/r53
        # refactor to check for None, and invoke closeRTP|RTCP if needed
    ...
    def receiveRTP(self, sock):
        try:
            ...
            logger.exception('receive RTP exception')
        # contributed by luke.weber@gmail.com, p2p-sip/r53
        # refactor to add closeRTP and reset rtpgen
        self._closeRTP()
        self._rtpgen = None
        try:
            os.close(fd)
    ...
    def sendRTP(self, data, dest=None):  # unline sendRTCP this is not a generator
        if self.rtp:
            ...
            if dest and dest[1] > 0 and dest[0] != '0.0.0.0':
                ...
                logger.debug('ignoring send RTP as dest is not set')
    ...
    def sendRTCP(self, data, dest=None):
        if self.rtcp:
            ...
            if dest and dest[1] > 0 and dest[0] != '0.0.0.0':
                ...
                logger.debug('ignoring send RTCP as dest is not set')
